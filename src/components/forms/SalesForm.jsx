...
export default function SalesForm({ open, onClose, products = [], onSubmit }) {
  const [customerName, setCustomerName] = useState("");
  const [items, setItems] = useState([{ id: crypto.randomUUID(), productId: "", quantity: 1 }]);
  ...
  const addItem = () => setItems((s) => [...s, { id: crypto.randomUUID(), productId: "", quantity: 1 }]);
  const removeItem = (id) => setItems((s) => (s.length > 1 ? s.filter((i) => i.id !== id) : s));
  const updateItem = (id, field, value) =>
  ...
  const saleTotal = useMemo(() => {
    return items.reduce((sum, it) => {
      const prod = products.find((p) => p.id === it.productId);
      if (!prod) return sum;
      return sum + prod.totalCostPerUnitKsh * (Number(it.quantity) || 0);
    }, 0);
  }, [items, products]);

  const handleSubmit = () => {
    if (!customerName.trim()) return alert("Please enter customer name.");
    if (items.some((it) => !it.productId)) return alert("Please select all product fields.");
        ...
        <div className="p-6 overflow-y-auto max-h-[85vh]">
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-4">
              <label className="text-sm font-semibold">Sale Items</label>
              {items.map((it) => (
                <div key={it.id} className="p-3 bg-gray-100 dark:bg-gray-800 rounded-xl border dark:border-gray-700 space-y-3">
                  <select
                    value={it.productId}
                  ...
                  >
                    ...
                    {products.map((p) => (
                      <option key={p.id} value={p.id}>
                        {p.productName} — Ksh {p.totalCostPerUnitKsh}
                      </option>
                    ))}
                  </select>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      onClick={() => updateItem(it.id, "quantity", Number(it.quantity) + 1)}
                      className="p-2 bg-gray-200 dark:bg-gray-700 rounded-lg"
                    >
                      <Plus size={16} />
                    </button>

                    <button onClick={() => removeItem(it.id)} className="ml-auto text-red-500 hover:text-red-700">
                      Remove
                    </button>
                  </div>
                </div>
