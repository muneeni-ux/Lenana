...
export default function OrderForm({ open, onClose, onSubmit, clients = [], products = [] }) {
  ...
  // Add a new empty item using backend product defaults
  const addItem = () => {
    ...
    setItems((prev) => [
      ...prev,
      {
        ...
        quantity: 1,
        unitPriceKsh: p.totalCostPerUnitKsh,
        productionCostPerUnitKsh: p.productionCostPerUnitKsh,
      },
    ]);
  };
  ...
  // When user selects a product, fetch its details from backend (already provided via products prop)
  ...
  // Auto-fill delivery address when client selected
  useEffect(() => {
    const client = clients.find((c) => c.id === clientId);
    if (client?.deliveryAddress) setDeliveryAddress(client.deliveryAddress);
  }, [clientId, clients]);
  ...
  // Order totals
  const orderTotals = useMemo(() => {
    const orderTotalKsh = lineTotals.reduce((sum, it) => sum + it.lineTotalKsh, 0);
    const totalProductionCostKsh = lineTotals.reduce((sum, it) => sum + it.lineProductionCostKsh, 0);
    const grossProfitKsh = orderTotalKsh - totalProductionCostKsh;
    const grossMarginPercent = orderTotalKsh ? (grossProfitKsh / orderTotalKsh) * 100 : 0;
    return { orderTotalKsh, totalProductionCostKsh, grossProfitKsh, grossMarginPercent };
  }, [lineTotals]);
  ...
  const validate = () => {
    ...
    if (!deliveryDate) return alert("Choose delivery date");
    if (!items.length || items.some((it) => !it.productId || it.quantity <= 0)) return alert("Add at least one valid item");
    return true;
  };

  const buildOrderPayload = (status) => ({
    clientId,
    clientName: clients.find((c) => c.id === clientId)?.businessName || "",
    deliveryAddress,
    deliveryDate,
    placementType,
    status,
    specialInstructions: instructions,
    customBrandingRequirements: branding,
    items: lineTotals.map((it) => ({
      productId: it.productId,
      productName: it.productName,
      quantity: Number(it.quantity),
      unitPriceKsh: Number(it.unitPriceKsh),
      productionCostPerUnitKsh: Number(it.productionCostPerUnitKsh),
      lineTotalKsh: Number(it.lineTotalKsh),
      lineProductionCostKsh: Number(it.lineProductionCostKsh),
      lineGrossProfitKsh: Number(it.lineGrossProfitKsh),
      lineMarginPercent: Number(it.lineMarginPercent),
    })),
    orderTotalKsh: orderTotals.orderTotalKsh,
    totalProductionCostKsh: orderTotals.totalProductionCostKsh,
    grossProfitKsh: orderTotals.grossProfitKsh,
  ...
  const handleSubmitOrder = (e) => {
    e.preventDefault();
    if (!validate()) return;
    onSubmit(buildOrderPayload("SUBMITTED"));
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="w-full max-w-3xl bg-white dark:bg-gray-900 rounded-2xl shadow-xl overflow-hidden max-h-[92vh]">
        ...
        <form onSubmit={handleSubmitOrder} className="overflow-y-auto max-h-[78vh] p-6 space-y-5">
          {/* client & delivery info */}
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-semibold">Client *</label>
              <select value={clientId} onChange={(e) => setClientId(e.target.value)} className="w-full mt-1 px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-800">
                <option value="">Select client</option>
            ...
            <div>
              <label className="text-sm font-semibold">Delivery Address *</label>
              <input value={deliveryAddress} onChange={(e) => setDeliveryAddress(e.target.value)} className="w-full mt-1 px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-800" />
            </div>

            <div>
              <label className="text-sm font-semibold">Delivery Date *</label>
              <input type="date" value={deliveryDate} onChange={(e) => setDeliveryDate(e.target.value)} className="w-full mt-1 px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-800" />
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-semibold">Order Items</h3>
              <button type="button" onClick={addItem} className="flex items-center gap-2 px-3 py-1 rounded-lg bg-green-600 text-white">
                <Plus size={16} /> Add Item
              </button>
            </div>
            <div className="space-y-3 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
              {lineTotals.map((it, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-center">
                  <div className="col-span-5">
                    <label className="text-xs text-gray-500">Product</label>
                    <select value={it.productId} onChange={(e) => onProductChange(i, e.target.value)} className="w-full mt-1 px-3 py-2 rounded-lg bg-white dark:bg-gray-700">
                      <option value="">Select product</option>
                  ...
                  <div className="col-span-2">
                    <label className="text-xs text-gray-500">Qty</label>
                    <div className="flex items-center gap-2 mt-1">
                      <button type="button" onClick={() => updateItem(i, { quantity: Math.max(1, it.quantity - 1) })} className="p-1 rounded bg-gray-100 dark:bg-gray-700"><Minus size={14} /></button>
                      <input type="number" value={it.quantity} min={1} onChange={(e) => updateItem(i, { quantity: Number(e.target.value) })} className="w-full text-center px-2 py-2 rounded-lg bg-white dark:bg-gray-700" />
                      <button type="button" onClick={() => updateItem(i, { quantity: it.quantity + 1 })} className="p-1 rounded bg-gray-100 dark:bg-gray-700"><Plus size={14} /></button>
                    </div>
                  </div>
                  <div className="col-span-2">
                    <label className="text-xs text-gray-500">Unit Price</label>
                    <input type="number" value={it.unitPriceKsh} readOnly className="w-full mt-1 px-3 py-2 rounded-lg bg-white dark:bg-gray-700" />
                  </div>
                  {/* <div className="col-span-2">
                    <label className="text-xs text-gray-500">Prod Cost</label>
                    <input type="number" value={it.productionCostPerUnitKsh} readOnly className="w-full mt-1 px-3 py-2 rounded-lg bg-white dark:bg-gray-700" />
                  </div> */}
                  <div className="col-span-1 text-right">
                    <label className="text-xs text-gray-500">Total</label>
                    <div className="mt-1 font-semibold">{it.lineTotalKsh.toLocaleString()}</div>
                    <button type="button" onClick={() => removeItem(i)} className="mt-2 text-red-600 hover:text-red-800"><Trash size={16} /></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg flex justify-between items-center">
            <div>Items: <strong>{lineTotals.length}</strong></div>
            <div>
              <div>Order Total: <strong>{orderTotals.orderTotalKsh.toLocaleString()}</strong></div>
              {/* <div className="text-sm text-gray-500">Gross Profit: {orderTotals.grossProfitKsh.toLocaleString()} ({orderTotals.grossMarginPercent.toFixed(1)}%)</div> */}
            </div>
          </div>
          ...
          {/* actions */}
          <div className="flex gap-3">
            <button type="button" onClick={handleSaveDraft} className="flex-1 bg-gray-300 dark:bg-gray-700 py-3 rounded-lg">Save Draft</button>
            <button type="submit" className="flex-1 bg-green-600 text-white py-3 rounded-lg">Submit Order</button>
          </div>
        </form>
      </div>
