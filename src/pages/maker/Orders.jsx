...
export default function Orders() {
  ...
  const defaultClients = [
    { id: "C1", businessName: "Mount Kenya Spa", phone: "+254712345678", deliveryAddress: "Nanyuki CBD" },
    { id: "C2", businessName: "Nanyuki Mart", phone: "+254723888444", deliveryAddress: "View Estate" },
    { id: "C3", businessName: "Laikipia Hotel", phone: "+254710000001", deliveryAddress: "Laikipia" },
  ];

  const defaultProducts = [
    { id: "P1", name: "20L Bottle", unitPriceKsh: 250, productionCostPerUnitKsh: 150 },
    { id: "P2", name: "10L Bottle", unitPriceKsh: 160, productionCostPerUnitKsh: 100 },
    { id: "P3", name: "Small Bottles Pack", unitPriceKsh: 120, productionCostPerUnitKsh: 70 },
  ];
  ...
  const seedOrders = [
    {
      ...
      placementType: "MANUAL_ENTRY",
      status: "DRAFT",
      items: [{ id: "oi-1", productId: "P1", productName: "20L Bottle", quantity: 12, unitPriceKsh: 250, productionCostPerUnitKsh: 150 }],
      specialInstructions: "",
  ...
  const [inventory, setInventory] = useLocalStorage("inventory", [
    ...
    { id: "INV-001", productId: "P1", quantityAvailable: 120, quantityReserved: 20, quantityDamaged: 5 },
    { id: "INV-002", productId: "P2", quantityAvailable: 60, quantityReserved: 15, quantityDamaged: 2 },
    { id: "INV-003", productId: "P3", quantityAvailable: 15, quantityReserved: 10, quantityDamaged: 3 },
  ]);
  ...
  const addOrder = (newOrder) => {
    ...
    const enriched = {
      ...newOrder,
      orderTotalKsh: newOrder.orderTotalKsh ?? (newOrder.items || []).reduce((s, it) => s + (it.quantity * it.unitPriceKsh), 0),
      totalProductionCostKsh: newOrder.totalProductionCostKsh ?? (newOrder.items || []).reduce((s, it) => s + (it.quantity * it.productionCostPerUnitKsh), 0),
  ...
  const markDelivered = (id) => {
    ...
    order.items.forEach((it) => {
      ...
      if (inv) {
        inv.quantityAvailable = Math.max(0, (inv.quantityAvailable || 0) - Number(it.quantity || 0));
      } else {
      ...
      prev.map((o) =>
        o.id === id
          ? {
              ...o,
              status: "DELIVERED",
              deliveryDate: o.deliveryDate || new Date().toISOString().split("T")[0],
            }
          : o
      )
    );
  ...
  return (
    <div className="pt-24 px-6 pb-12 text-gray-800 dark:text-gray-100 transition-all max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold">Your Orders</h1>
          <p className="text-gray-500">Create, manage and submit orders. (Checkers handle approvals.)</p>
        </div>
      ...
      <div className="mb-6 flex items-center gap-4">
        <label className="font-semibold">Filter by Status:</label>
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-4 py-2 rounded-lg bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700">
            ...
            {orders
              ...
              .map((order) => (
                <tr key={order.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                  ...
                  <td className="p-3">{getStatusBadge(order.status)}</td>
                  <td className="p-3">{order.deliveryDate || "---"}</td>
                  <td className="p-3 font-bold">{Number(order.orderTotalKsh || 0).toLocaleString()}</td>

                  <td className="p-3">
                    {/* DRAFT → Save as DRAFT (handled by modal) or Submit */}
                    {order.status === "DRAFT" && (
                      <button onClick={() => submitDraft(order.id)} className="px-3 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-1">
                        <Send size={16} /> Submit
                      </button>
                    )}
                    ...
                    {/* SUBMITTED */}
                    {order.status === "SUBMITTED" && (
                      <span className="text-yellow-700 dark:text-yellow-300">Awaiting Checker</span>
                    )}

                    {/* APPROVED → deliver (maker can mark delivered here if desired) */}
                    {order.status === "APPROVED" && (
                      <button onClick={() => markDelivered(order.id)} className="px-3 py-1 bg-green-600 text-white rounded-lg hover:bg-green-700">
                        Mark Delivered
                      </button>
                    )}
