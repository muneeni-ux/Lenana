    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="w-full max-w-lg bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 overflow-auto">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-300 mb-3">
          Explain why this production batch is rejected. Reason is saved for
          audit.
        </p>
...
function QCModal({ open, onClose, batch, onSubmitQC, currentUser }) {
  ...
  const passed = Math.max(
    0,
    qtyCompleted - (isNaN(defectiveNum) ? 0 : defectiveNum)
  );
...
export default function ManageProduction() {
  ...
  const fetchBatches = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(`${SERVER_URL}/api/production`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setBatches(res.data || []);
    } catch (err) {
  ...
  const filtered = useMemo(() => {
    return batches
      .filter((b) => {
        ...
        if (productFilter !== "All" && b.productId !== productFilter)
          return false;
        const s = search.trim().toLowerCase();
        if (
          s &&
          !`${b.batchId} ${b.product} ${b.createdBy}`.toLowerCase().includes(s)
        )
          return false;
        if (dateFrom && new Date(b.productionDate) < new Date(dateFrom))
          return false;
        if (dateTo && new Date(b.productionDate) > new Date(dateTo))
          return false;
        return true;
      })
      .sort((a, z) => {
        if (sortBy === "newest")
          return new Date(z.productionDate) - new Date(a.productionDate);
        if (sortBy === "oldest")
          return new Date(a.productionDate) - new Date(z.productionDate);
  ...
  const handleSubmitQC = async ({
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `${SERVER_URL}/api/production/${qcTargetBatch.batchId}/qc-approve`,
        { defectiveQty, passedQty, qcNotes },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast.success("Batch QC approved!");
      fetchBatches();
      setQcModalOpen(false);
      setQcTargetBatch(null);
    } catch (err) {
      console.error(err);
      toast.error(err.response?.data?.message || "Failed to approve QC");
    }
  };

  // Reject batch
  const finalizeReject = async (reason) => {
    if (!selectedBatch) return;
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `${SERVER_URL}/api/production/${selectedBatch.batchId}/reject`,
  ...
  const openQcIfEligible = (batch) => {
    if (batch.status !== "COMPLETED") {
      alert("Batch must be COMPLETED before QC.");
      return;
    }
  ...
  const onApproveClick = (batch) => openQcIfEligible(batch);
  const beginReject = (batch) => {
    setSelectedBatch(batch);
    setRejectModalOpen(true);
  };
  const openDetails = (batch) => setSelectedBatch(batch);
  ...
  return (
    <div className="pt-16 px-6 pb-10 text-gray-800 dark:text-gray-100 transition-all max-w-7xl mx-auto">
      ...
      <div className="flex items-start justify-between gap-6 mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Approve QC for completed batches, or mark batches as rejected. QC
            approval will record defects and update inventory automatically.
          </p>
        </div>
              ...
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="ml-2 text-sm px-2 py-1 rounded bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">
                      {b.batchId}
                    </span>
                  </div>
                  ...
                  <div className="text-sm text-gray-500 dark:text-gray-300 mt-2">
                    ...
                    <strong>{b.quantityCompleted ?? 0}</strong> · Date:{" "}
                    {formatDateTime(b.productionDate)}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="mt-2 flex flex-col gap-2">
                      ...
                      <button
                        ...
                        disabled={b.status !== "COMPLETED"} // only COMPLETED batches
                        title={
                          b.status === "APPROVED" || b.status === "REJECTED"
                            ? "This batch is already finalized"
                            : b.status !== "COMPLETED"
                            ? "Only COMPLETED batches can be QC-approved"
                            : "Open QC modal to approve"
                        }
                      ...
                      >
                        <XCircle size={14} className="inline mr-2" /> Mark
                        Rejected
                      </button>
                    </div>
                  </div>
        ...
        <aside className="bg-white dark:bg-gray-900 p-4 rounded-xl shadow space-y-4">
          ...
          {!selectedBatch && (
            <div className="text-gray-500">
              Select a batch to see details, QC notes and inventory snapshot.
            </div>
          )}
            ...
            <>
              <div>
                ...
                <div className="mt-2 text-sm">
                  Planned: {selectedBatch.quantityPlanned}
                </div>
                <div className="text-sm">
                  Produced: {selectedBatch.quantityCompleted ?? 0}
                </div>
                <div className="text-sm">
                  Date: {formatDateTime(selectedBatch.productionDate)}
                </div>
                ...
                {selectedBatch.qualityCheckPassed && (
                  <div className="mt-3 p-3 rounded bg-green-50 dark:bg-green-900/20 text-green-700">
                    ...
                    <div className="text-sm mt-1">
                      Passed: {selectedBatch.passedQty ?? 0} • Defective:{" "}
                      {selectedBatch.defectiveQty ?? 0}
                      <div className="text-xs text-gray-500">
                        Checked by: {selectedBatch.qcBy} on{" "}
                        {selectedBatch.qcDate}
                      </div>
                      {selectedBatch.qcNotes && (
                        <div className="mt-1 text-sm">
                          Notes: {selectedBatch.qcNotes}
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="pt-3 border-t dark:border-gray-700">
                ...
                {(() => {
                  const inv = inventory.find(
                    (i) => i.productId === selectedBatch.productId
                  );
                  if (!inv)
                    return (
                      <div className="text-sm text-gray-500">
                  ...
                  return (
                    <div className="text-sm">
                      <div>
                        Available: <strong>{inv.quantityAvailable}</strong>
                      </div>
                      <div>
                        Reserved: <strong>{inv.quantityReserved}</strong>
                      </div>
                      <div>
                        Damaged:{" "}
                      ...
                      <div className="mt-2">
                        Total On Hand: <strong>{total}</strong>
                      </div>
                      <div className="text-xs text-gray-500 mt-1">
                        Last count: {inv.lastStockCountDate}
                      </div>
                    </div>
                  );
                })()}
              ...
              <div className="pt-3 border-t dark:border-gray-700 flex gap-2">
                ...
                <button
                  onClick={() => navigate("/checker/production")}
      ...
      <QCModal
        ...
        currentUser={
          JSON.parse(localStorage.getItem("user"))?.username || "checker"
        }
